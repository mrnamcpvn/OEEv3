...
export class DowntimeReasonsComponent implements OnInit, AfterViewInit {
  ...
  loadBuilding() {
    this.commonService.getBuilding(this.factory).subscribe(
      res => {
  ...
  openModal(template: TemplateRef<any>, item: ChartReason) {
    this.loadReason();
    this.downtimeReasonsService.getReasons(item.id).subscribe(res => {
      if (res != null) {
  ...
  changeReason(event: any) {
    this.loadReason2(this.reason_1);
  }
  ...
  updateDate(event: any) {
    if (
      formatDate(this.date, "yyyy-MM-dd", "en-US") !=
      formatDate(new Date(event.srcElement.value), "yyyy-MM-dd", "en-US")
    ) {
  ...
  loadMachine() {
    this.commonService
      .getMachinesActionTime(this.factory, this.building, this.machine_type)
      .subscribe(
        res => {
  ...
  loadReason2(value) {
    this.downtimeReasonsService.getDowntimeReasonDetail(value).subscribe(
      res => {
        if (res != null) {
          this.reason2s = res.map(item => {
