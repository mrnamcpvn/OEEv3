...
namespace OEE_API.Helpers
{
    public class PageListUtility<T> where T : class
    {
        ...
        public static async Task<PageListUtility<T>> PageListAsyncChartReason(List<T> source, List<T> sourceT, string machineName ,int pageNumber, int pageSize = 10)
        {
            var count =  sourceT.Count();
            int skip = (pageNumber - 1) * pageSize;
            var items =  sourceT.Skip(skip).Take(pageSize).ToList();
            var itemsC =  source.ToList();
            return new PageListUtility<T>(itemsC, items, machineName, count, pageNumber, pageSize, skip);
        }
